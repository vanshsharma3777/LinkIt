...
import { Menu, Check, ExternalLink, Heading, Globe, AlignLeft, Text, Pencil, Trash2, Calendar, Tag as TagIcon, Hash, Type } from "lucide-react"
    ...
    <div className="min-h-screen bg-[#0F1115] pb-10 text-gray-200 font-sans">
      <nav className="sticky top-0 z-40 bg-[#0F1115]/80 backdrop-blur-md border-b border-[#1f2937] px-4 md:px-8 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div
            className="text-xl md:text-2xl font-black tracking-tighter text-orange-500 cursor-pointer"
            onClick={() => nav("/")}
          >
            LinkIT
          </div>
          <div className="relative flex-1 max-w-md">
            <input
              ...
              className="w-full rounded-full bg-[#121826] px-5 py-2 text-sm text-gray-300 outline-none border border-[#1f2937] focus:border-orange-500 transition-all"
            />
            <button
          ...
          >
            <AnimatePresence mode="popLayout">
              {filteredLinks.map((link, index) => (
                <motion.div
                  // Use link.id if it exists, otherwise fallback to index
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2 overflow-hidden">
                        {/* Added Type Icon back in */}
                        <Type size={18} className="text-orange-500 shrink-0" />
                        <h3 className="text-xl font-bold text-gray-100 uppercase tracking-tight line-clamp-1 group-hover:text-orange-500 transition-colors">
                          {link.title}
                        </h3>
                      </div>
                      <div className="flex gap-3 shrink-0">
                        <button onClick={() => nav(`/edit/${link.id}`)} className="text-gray-500 hover:text-green-400 transition-colors">
                          <Pencil size={18} />
                        </button>
                        <button onClick={() => setDeleteId(link.id)} className="text-gray-500 hover:text-red-400 transition-colors">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {link.tags && link.tags.length > 0 ? (
                          link.tags.map((tag, tagIndex) => (
                            <span
                              // FIX: Using name + index since you have no tag IDs
